...
export default function UniverseView({
  ...
  useEffect(() => {
    ...
    // Add a glow to the sun using our inline function
    const sunGlowGeometry = new THREE.SphereGeometry(2.5, 32, 32)
    const sunGlowMaterial = createGlowMaterial({
      color: 0xffcc00,
      power: 2.0,
      size: 0.7,
      opacity: 0.7,
    ...
    const planets = planetConfigs.map((config) => {
      ...
      // Glow halo mesh using our inline function
      ...
      planets.forEach((planet) => {
        ...
        if (planet.name === hoveredPlanet) {
          ...
          if (material && material.uniforms) {
            material.uniforms.glowPower.value = 2.5
  ...
  }, [handleClick, handleMouseMove, hoveredPlanet, activeZone])
